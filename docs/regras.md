...
- **Cadastro de pessoas:** adicionar pede e-mail do domínio da `Config`, nome de exibição (sugerido pelo e-mail), setor principal, perfil e data de entrada. Ninguém é apagado — sai da equipe = `Ativo = Não`. Enquanto estiver inativa, **os lançamentos dela de hoje em diante somem das telas e deixam de contar** (os dias passados continuam, como histórico) (escalas, contagem do sábado, cobertura de setor, vagas, fila do home office, linha do tempo de férias e trocas pendentes); as linhas continuam na planilha e **voltam a aparecer se ela for reativada**. Quem não está no cadastro continua aparecendo, para um lançamento antigo não sumir sem explicação. Inativar alguém com lançamentos futuros ou que seja o único gestor ativo gera aviso. O gestor também troca o **setor principal** e marca quantos **setores extras** quiser pela mesma janela (botão "Setores"); as abas `Pessoas` e `Setores extras` são atualizadas juntas.
